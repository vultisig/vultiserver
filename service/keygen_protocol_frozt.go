...
func NewFroztKeygenProtocol(
	...
	session, err := frozt.DkgSessionFromSetup(setupMsg, []byte(localPartyID))
	if err != nil {
...
func (p *FroztKeygenProtocol) drainRemainingOutbound() error {
	for {
		msg, err := frozt.DkgSessionTakeMsg(p.session)
		if err != nil {
			return fmt.Errorf("frozt keygen take msg: %w", err)
		}
		if len(msg) == 0 {
			return nil
		}
