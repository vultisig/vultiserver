...
func NewFromtReshareProtocol(
	...
	oldKeyPackage, err := fromt.KeyShareBundleKeyPackage(oldKeyShare)
	if err != nil {
		return nil, fmt.Errorf("fromt unpack old key package: %w", err)
	}

	session, err := fromt.ReshareSessionFromSetup(setupMsg, []byte(localPartyID), oldKeyPackage)
	if err != nil {
...
func (p *FromtReshareProtocol) drainRemainingOutbound() error {
	for {
		msg, err := fromt.ReshareSessionTakeMsg(p.session)
		if err != nil {
			return fmt.Errorf("fromt reshare take msg: %w", err)
		}
		if len(msg) == 0 {
			return nil
		}
...
func (p *FromtReshareProtocol) computeResult() (*PhaseResult, error) {
	...
	newKeyPackage, newPubKeyPackage, err := fromt.ReshareSessionResult(p.session)
	if err != nil {
		return nil, fmt.Errorf("fromt reshare result: %w", err)
	}

	viewKey, err := fromt.KeyShareViewKey(p.oldKeyShare)
	if err != nil {
		return nil, fmt.Errorf("fromt view key: %w", err)
	}

	network, err := fromt.KeyShareNetwork(p.oldKeyShare)
	if err != nil {
		return nil, fmt.Errorf("fromt network: %w", err)
	}

	birthday, err := fromt.KeyShareBirthday(p.oldKeyShare)
	if err != nil {
		return nil, fmt.Errorf("fromt birthday: %w", err)
	}

	newKeyShare, err := fromt.KeyShareBundlePack(newKeyPackage, newPubKeyPackage, viewKey, network, birthday)
	if err != nil {
		return nil, fmt.Errorf("fromt repack bundle: %w", err)
	}
